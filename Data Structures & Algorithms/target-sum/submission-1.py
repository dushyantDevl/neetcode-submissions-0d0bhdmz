class Solution:
    ...
    def _countPartitions(self, arr, diff):
        ...
        prev, now = [0] * (target + 1), [0] * (target + 1)

        # Base row (idx == 0), mirroring the recursive base case
        for curr in range(target + 1):
            if curr == arr[0] == 0:
                prev[curr] = 2          # take the zero, or don't
            elif curr == 0 or arr[0] == curr:
                prev[curr] = 1          # empty choice, or take arr[0]
            else:
                prev[curr] = 0
        ...
        for idx in range(1, n):
            for curr in range(target + 1):
                notTake = prev[curr]
                take = prev[curr - arr[idx]] if curr >= arr[idx] else 0
                now[curr] = take + notTake
            prev = now[:]
